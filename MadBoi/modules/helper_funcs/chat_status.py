    ...
    def is_dev_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user.id in DEV_USERS:
            ...
            update.effective_message.reply_text(
                "The command you are trying to acess,"
                " is only for DEVs of this bot, You can't use it."
            )
...
def sudo_plus(func):
    ...
    def is_sudo_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_sudo_plus(chat, user.id):
            ...
            update.effective_message.reply_text(
                "The command you tried to access is allowed only for Bot DEVs.\nYou can't use it, Thank You 💝💝💝💝"
            )
...
def whitelist_plus(func):
    ...
    def is_whitelist_plus_func(
        ...
        if user and is_whitelist_plus(chat, user.id):
            ...
            update.effective_message.reply_text(
                f"You don't have access to use this.\n\nVisit @Romeo_JulietBotSupport for further queries."
            )
...
from MadBoi.modules import connection
