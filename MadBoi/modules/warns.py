...
from MadBoi import TIGERS, WOLVES, dispatcher
...
from MadBoi.modules.helper_funcs.string_handling import split_quotes
from MadBoi.modules.log_channel import loggable
from MadBoi.modules.sql import warns_sql as sql
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>➥ Admin:</b> {mention_html(user.id, user.first_name)}\n"
                f"Has disabled strong warns. I will use normal kick on users."
            )
